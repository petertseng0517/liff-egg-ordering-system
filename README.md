...
| **商品管理介面** | 統一管理商品、單價、庫存、分類，支援即時編輯與庫存警告 |
...
- 直接編輯會員資料
- 修改即時同步至 Firestore
- **管理者自行新增會員** ✨：針對電話訂購、現場訂購等非 LINE 註冊客戶，管理者可直接在後台建立會員資料，系統自動產生暫時帳號（`ADMIN_xxx` 格式）
- **LINE 帳號綁定驗證** ✨：對暫時帳號可生成專屬驗證連結（`https://liff.line.me/...`），客戶於 LINE 內點擊連結並輸入手機號碼完成二次驗證後，系統會建立其真實 LINE User ID 的正式會員資料，原暫時帳號自動標記為「已綁定-略」
- **會員狀態管理**：啟用 / 停用 / 黑名單 / 已刪除 / 已綁定-略，列表預設僅顯示「啟用」會員，可透過篩選查看其他狀態
...
ADMIN_ACCOUNTS=your_admin:CHANGE_ME,your_manager:CHANGE_ME_TOO
...
```javascript
{
  userId: "U1234567890...",        // LINE User ID；管理者新增的暫時帳號為 "ADMIN_時間戳_隨機碼" 格式
  name: "范國紅",
  phone: "0911351882",
  address: "新竹市...",
  address2: "台北市...",           // 備用地址
  birthDate: "1990-01-15",
  status: "啟用",                  // 啟用/停用/黑名單/已刪除/已綁定-略（LINE 綁定後舊暫時帳號的標記）
  createdAt: Timestamp,
  updatedAt: Timestamp
}
```
...
#### `verificationTokens` - LINE 帳號綁定驗證令牌
```javascript
{
  userId: "ADMIN_1782046430_E261",  // 對應的暫時帳號
  createdAt: Timestamp,
  expiresAt: Timestamp              // 24 小時後過期，驗證成功或過期後會自動刪除
}
```

#### 其他集合
- `products` - 商品資訊（名稱、單價、分類、庫存）
- `stockLogs` - 庫存異動紀錄
- `categories` - 商品分類
- `stockAlerts` - 庫存警告設定
...
## 📈 最近更新（v3.4 - 2026-06-22）

### ✨ 新功能
- **管理者自行新增會員**
  - 針對電話、現場訂購等非 LINE 註冊客戶，後台可直接建立會員資料
  - 系統自動產生暫時帳號（`ADMIN_xxx` 格式），不需要客戶事先提供 LINE ID
- **LINE 帳號綁定驗證**
  - 後台「ID 驗證工具」可生成 `https://liff.line.me/...` 格式的綁定連結
  - 客戶於 LINE 內點擊連結，輸入手機號碼完成二次驗證後，自動建立其真實 LINE User ID 的正式會員資料
  - 手機號碼二次驗證可防止連結被轉傳或截獲後遭冒用綁定
- **會員 / 訂單軟刪除機制**
  - 會員與訂單新增「已刪除」狀態，預設列表自動隱藏，篩選後仍可查詢，資料不會真正遺失
  - 會員列表預設篩選改為僅顯示「啟用」會員

### 🔧 修改
- LINE 綁定成功後，原暫時帳號標記為專屬的「已綁定-略」狀態，與真正刪除的會員區分

### 🗑️ 已移除功能
- ❌ Google Sheets 備用資料庫後端與相關搬遷腳本，統一改用 Firestore
...
### 🔧 修改
- **移除折扣功能** - 原先的折扣規則已剔除，改用商品管理介面統一定價
- 訂單計算邏輯簡化，基於商品單價直接計算金額
- 前端訂購介面改用商品管理配置的商品資訊

### 🗑️ 已移除功能
- ❌ 折扣規則管理（Discounts Collection）
- ❌ 批量優惠組配置
- ❌ 複雜的折扣邏輯
...
A：登入後台 → 點擊導航欄「🛒 商品管理」 → 可新增/編輯/刪除商品，設定單價、庫存數量、分類等。

**Q：可以設定折扣嗎？**

A：折扣功能已移除。現在使用商品管理介面統一設定商品單價，訂單金額基於商品的實際單價計算。

**Q：客戶沒有用 LINE 訂購過，要怎麼幫他建立會員資料？**

A：登入後台 → 「會員管理」→ 點擊「新增會員」，填寫姓名、手機、地址即可。系統會產生一個 `ADMIN_xxx` 格式的暫時帳號，之後客戶用 LINE 完成綁定驗證後會自動轉為正式帳號。

**Q：如何讓客戶把暫時帳號綁定到他自己的 LINE？**

A：在會員列表找到該筆 `ADMIN_xxx` 帳號 → 點擊「驗證」→「生成驗證連結」，將連結傳給客戶（建議透過 LINE 訊息傳送，因為連結必須在 LINE App 內開啟才能成功）。客戶點擊連結後，需輸入手機號碼進行二次驗證，驗證通過才會完成綁定。

**Q：綁定後，原本的 `ADMIN_xxx` 帳號會被刪除嗎？**

A：不會被刪除，狀態會自動改為「已綁定-略」，預設列表不顯示，但資料保留（避免影響該帳號名下的歷史訂單）。篩選狀態選「已綁定-略」即可查到。
